...
public class PushNotificationReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (intent.getAction().equals(ACTION_PUSH)) {
            ...
            LayerClient layerClient = MyApplication.getLayerClient();
...
//            String conversationTitle = ConversationsListFragment.chatAvatarModelHashMap.get(Util.getConversationTitle(MyApplication.getLayerClient(), MyApplication.getParticipantProvider(), conversation)).getFirst_Name();
            String conversationTitle = Util.getConversationTitle(MyApplication.getLayerClient(), MyApplication.getParticipantProvider(), conversation);
//            String[] conversationDetails = conversationTitle.split("&");
//            conversationTitle = conversationDetails[1];
            NotificationCompat.InboxStyle inboxStyle = new NotificationCompat.InboxStyle().setBigContentTitle("The Bureau");
            int i;
            ...
            while (i < positions.size()) {
                String norificationText = positionText.get(positions.get(i++));
/*
                if(norificationText.contains("Attachment from")){
                    String strNotification = "";
                    String temp[] = norificationText.split(" ");
                    for (int j = 2; j < temp.length; j++){
                        if(j == 2) {
                            String userId = temp[j].replace(":","");
                            strNotification = strNotification + AppData.getString(context, userId)+":";
                        }else{
                            strNotification = strNotification + " " + temp[j];
                        }
                    }
                    inboxStyle.addLine(strNotification);
                }else {
                    inboxStyle.addLine(norificationText);
                }
*/
           ...
           /* if(collapsedSummary.contains("Attachment from")){
                String strNotification = "";
                String temp[] = collapsedSummary.split(" ");
                for (int j = 2; j < temp.length; j++){
                    if(j == 2) {
                        String userId = temp[j].replace(":","");
//                        strNotification = strNotification + AppData.getString(context, userId)+":";
                        strNotification = strNotification +  userId+":";
                    }else{
                        strNotification = strNotification + " " + temp[j];
                    }
                }
                collapsedSummary = strNotification;
            }*/
        ...
        private void notifyOnContentFailure(Context context, Uri conversationId, Uri messageId, String text) {
         ...
         /*   if(text.contains("Attachment from")){
                String strNotification = "";
            ...
            NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(context)
                    .setSmallIcon(R.mipmap.ic_launcher)
                    .setContentTitle("TheBureau")//context.getString(R.string.push_notification_no_content_title))
                    .setContentText(text)
                    .setAutoCancel(true)
                    .setLights(context.getResources().getColor(R.color.atlas_action_bar_background), 100, 1900)
                    .setPriority(NotificationCompat.PRIORITY_HIGH)
